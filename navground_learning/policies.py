...
class OrderInvariantCombinedExtractor(BaseFeaturesExtractor):
    ...
    :param replicated_keys:  additional keys to add to the ordering invariant groups, replicating the values for each group
    :param filter_key: the key to use for masking to select items with positive values of this key
    :param removed_keys: keys removed from the observations before concatenating with ordering invariant features
        ...
        for key, subspace in observation_space.spaces.items():
            if is_image_space(subspace, normalized_image=normalized_image):
                ...
                if key in order_invariant_keys:
                    order_invariant_lens.add(cast(spaces.Box, subspace).shape[0])
...
class OrderInvariantFlattenExtractor(BaseFeaturesExtractor):
    ...
    Similar to `OrderInvariantCombinedExtractor` for flat observation spaces.
    ...
    :param order_invariant_slices: the slices to group together and process by an ordering invariant feature extractor
    :param replicated_slices:  additional slices to add to the ordering invariant groups, replicating the values for each group
    :param filter_slice: the slice to use for masking to select items with positive values for indices in this slice
    :param removed_slices: keys removed from the observations before concatenating with ordering invariant features
    :param net_arch: the ordering invariant MLP layers sizes
    :param activation_fn: the ordering invariant MLP activation function
    """
    ...
    def forward(self, observations: th.Tensor) -> th.Tensor:
        ...
        if self._invariant_slices:
            ...
            ois = [observations[..., s].reshape(*shape[:-1], self._number, -1)
                   for s in self._invariant_slices]
            if self._should_replicate:
                ri = [1] * (len(shape) + 1)
                ri[-2] = self._number
                r = observations[..., self._replicated_mask].reshape(*shape[:-1], 1, -1)
            ...
            if len(oi):
                ...
                if self.use_masked_tensors and mask is not None:
                    ...
                    for reduction in self.reductions:
                        ...
                        zs.append(r.to_tensor(0))  # type: ignore
...
def make_order_invariant_flatten_extractor(
    ...
    ns = set(cast(spaces.Box, dict_space[k]).shape[0] for k in order_invariant_keys)
    if filter_key and filter_key in dict_space:
        ns.add(cast(spaces.Box, dict_space[filter_key]).shape[0])
    assert len(ns) == 1
    number = list(ns)[0]
    indices = {}
    i = 0
    for k, v in dict_space.items():
        dim = spaces.flatdim(v)
        indices[k] = slice(i, i + dim, 1)
        i += dim
    invariant_slices = [indices[k] for k in order_invariant_keys if k in indices]
    ...
    filter_slice = indices.get(filter_key, None)
